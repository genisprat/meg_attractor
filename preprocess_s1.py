...
#    for ii in [1]:
#        filename='/mnt/homes/home024/gortega/megdata/S1-6_Attractor_20161024_01.ds'
        if filename==path_cluster+'/megdata/S1-7_Attractor_20161025_01.ds':
            aux_aux=True
        else:
            aux_aux=False
        date=filename[-14:-6]
        ...
        for i, ((bnum, mb), (_, tb)) in enumerate(zip(meta.groupby(index), timing.groupby(index))):
            if bnum==4 and aux_aux:
                ...
                if len(rlmeta)>0:
                    ...
                    resplock.save(path_cluster+'preprocess_data/resp_sub'+str(subject)+'_s'+str(int(mb.session_number[0]))+'_block'+str(int(mb.block_start[0]))+'-epo.fif.gz')
