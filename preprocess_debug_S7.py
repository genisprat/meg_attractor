...
#        date=20161129
    ...
    #    I created the key_map to give a number to each trial.
        ...
        print 'len_meta_total',len(meta)
        bnum=3
        mb=meta[meta.block_start==bnum]
        tb=timing[mb.index[0]:mb.index[-1]+1]
        i=bnum-1
        ...
        mb2, tb2 = meg.preprocessing.get_meta(r, mapping, trial_pins, 150, 151, other_pins) #get new metadata for each block
        print 'metadata done, preprocesing...',mb2.session_number.iloc[0]
...
#        r, ants, artdef = meg.preprocessing.preprocess_block(r, blinks=True) #preprocess of each block looking for artifacts
        ...
        print 'len_meta_block ',len(mb),len(mb2)
#    
        r, ants, artdef = meg.preprocessing.preprocess_block(r, blinks=True)  
        trial_repeat=[]
        cache={}
        mb2.loc[:,'hash']=1
        for j in mb2.index:
            mb2.loc[j,'hash'],aux=hash4([subject, int(mb2.session_number[j]),int(bnum),int(mb2.trial_num[j]),int(date)])
            trial_repeat.append(aux)
        rlmeta, resplock = meg.preprocessing.get_epoch(r, mb2.dropna(), tb2.dropna(), event='response_time', epoch_time=(-2.5, .5),
...
#                r, ants, artdef = meg.preprocessing.preprocess_block(r, blinks=True) #preprocess of each block looking for artifacts
#                #created hash: in thios column I will indentify the for each trial a different number.
#                print 'len_meta_block ',bnum,len(mb)
#    
#                if len(r.info['bads'])>0:
#                    r.load_data()
#                    r.interpolate_bads(reset_bads=False)
#                    print 'bad_channel',r.info['bads']
#                    r.info['bads']=[]
#                    
#                
##                trial_repeat=[]
