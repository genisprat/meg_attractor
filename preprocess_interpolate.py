...
#    subject=int(subjects[int(sys.argv[1])])
    ...
    path=path_hamburg+'megdata/'
    
    block=1
    
    for filename in glob.glob(os.path.join(path, '*S'+str(subject)+'-5*01.ds')):
#    for ii in [1]:
...
#        date=20161129
    ...
    #    I created the key_map to give a number to each trial.
        ...
        events=mne.find_events(raw, 'UPPT001', shortest_event=1) #list of events and times
        ...
        mb=meta[meta.block_start==block] #metya data and timming for one block
        tb=timing.loc[mb.index]           
                
        r = raw.copy() #created a copy to do not change raw
        r.crop(tmin=time_block[block-1]/1200.0-10, tmax=1+(tb.feedback_time.max()/1200.)) #crop for each block
